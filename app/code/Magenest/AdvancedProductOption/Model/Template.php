...
class Template extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function getOptionsWithFullInfoFrontend()
    {
        ...
        $options           = [];
        $templateId        = $this->getId();
        $optionsCollection = $this->optionFactory->create()->getCollection()->addFieldToFilter('template_id', $templateId)->setOrder('main_table.order','asc');
        //->setOrder('order', "asc");
      // $optionsCollection->setOrder('order','asc');
        if ($optionsCollection->getSize() > 0) {
            /*
