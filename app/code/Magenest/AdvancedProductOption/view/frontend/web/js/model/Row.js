        ...
        return function (data) {
            ...
            return {
                ...
                chooseOptionFromRow : function (row) {
                    ...
                    if (self.option_type == 'radio') {
                        if (row.checkSelected()) {
                            jquery('input[type="radio"]').each(function (i) {
                                if (row.checkSelected() == jquery(this).val()) {
                                    jquery(this).attr('data-option', 'checked');
                                } else if (aop === this.name){
                                    jquery(this).attr('data-option', 'nochecked');
                                }
                            });
                        }
                    }
                    ...
                    var rowIds = [];
                    jquery('.advanced-product-option-field').each(function (i) {
                        rowIds[i] = jquery(this).val();
                    });
                    var x = 0;
                    if (typeof rowIds !== 'undefined' && rowIds.length > 0) {
                        x = rowIds.length;
                    }
                    jquery('.advanced-product-row-field').each(function (i) {
                        var atri = jquery(this).attr('data-option');
                        if(atri == 'checked'){
                            var y = x+i;
                            rowIds[y] = jquery(this).val();
                        }
                    });
                    ...
                    if (rowIds.length == 0) {
                        ...
                        rowIds.forEach(function (element) {
						  ...
						  //if (self.option_type == 'radio') { flag = 1; }
                        if(flag){
                            ...
                            self.price(0);
                                ...
                                jquery('#advanced_custom_option_price_cal').val(productPrice);
                                jquery('.box-tocart').hide();
                    jquery('.price-box').trigger('updatePrice', {
                        'prices': {
                    ...
                    if (typeof rowIds !== 'undefined' && rowIds.length > 0) {
                        ko.utils.arrayForEach(self.model.ao(), function (optionItem) {
                            ...
                            if (self.model.originProductPrice == -1) {
                                var priceHtml =  jquery('.product-info-price').find('span.price').html();
